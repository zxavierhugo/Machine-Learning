...
%of a restaurant franchise and are considering different cities for opening 
%a new outlet. The chain already has trucks in various cities and you have 
%data for profits and populations from the cities.


fprintf('Plotting Data ...\n') % the dataset is loaded from the data file into 
...
%We store each example as a row in the the X matrix in Octave/MATLAB. 
%To take into account the intercept term (θ0), we add an additional first 
%column to X and set it to all ones. This allows us to treat θ0 as simply 
%another ‘feature’.
...
J = computeCost(X, y, [1 ; -2]);
fprintf('\nWith theta = [1 ; -2]\nCost computed = %f\n', J);
%fprintf('Expected cost value (approx) 304\n');
...
%by the vector θ, not X and y. That is, we minimize the value of J(θ) by 
%changing the values of the vector θ, not by changing X or y. Refer to the 
...
%starter code for gradientDescent.m calls computeCost on every iteration and 
%prints the cost. Assuming you have implemented gradient descent and 
...
%linear fit. The result should look something like Figure 2: Your final values 
%for θ will also be used to make predictions on profits in areas of 35,000 
%and 70,000 people. Note the way that the following lines in ex1.m uses matrix 
%multiplication, rather than explicit summation or looping, to calculate the 
%predictions. This is an example of code vectorization in Octave/MATLAB.
...
% values for population sizes of 35,000 and 70,000
